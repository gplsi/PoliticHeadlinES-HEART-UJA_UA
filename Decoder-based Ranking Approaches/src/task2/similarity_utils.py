...
This module provides three complementary strategies for matching a test article
against a pool of training examples so that the most relevant ones can be
injected into the LLM prompt as few-shot exemplars:

  • TF-IDF (Jaccard token overlap) — Fast, deterministic, requires no external
    API calls. Good for lexical matches and works offline.

  • Semantic (embedding cosine similarity) — Uses dense vector representations
    from an OpenAI-compatible embedding model. Captures synonymy and paraphrase
    beyond exact word overlap, but requires API access.

  • Hybrid RRF (Reciprocal Rank Fusion) — Combines the rankings produced by
    TF-IDF and semantic similarity via a weighted RRF formula. This typically
    yields more robust retrieval than either signal alone.

The typical call chain is:
  1. ``load_training_data`` reads a CSV into row dictionaries.
  2. ``cache_embeddings`` pre-computes and persists embeddings for the training
     set so that ``select_similar_examples`` does not re-encode on every row.
  3. ``select_similar_examples`` scores every training article against the test
     article and returns the top ``num_examples`` rows.
  4. ``build_fewshot_examples_for_prompt`` formats the selected rows into a
     Spanish-language prompt block that the main pipeline injects before the
     target article.

All three metrics are implemented to gracefully degrade:
  - If embedding retrieval fails, hybrid/semantic modes fall back to TF-IDF.
  - Missing embeddings are replaced by zero vectors so scoring can continue.
  - Empty or very short texts yield a similarity of 0.0 rather than crashing.
"""
...
def cache_embeddings(
    ...
    # Batching could be added if the embedding endpoint supports multi-input.
    ...
    for idx, row in enumerate(training_data):
        ...
        # Use a zero vector as a safe placeholder when embedding fails,
        # so downstream cosine similarity simply yields 0.0 for this row.
        embeddings.append(embedding if embedding else [0.0] * 1536)

        # Progress logging every 10 examples to keep the user informed.
        if (idx + 1) % 10 == 0:
            print(f"  ... {idx + 1}/{len(training_data)} embeddings computed")

    # Persist the cache for future runs so that restarting the pipeline
    # does not require re-encoding the entire training set.
    try:
...
def select_similar_examples(
    ...
    # ------------------------------------------------------------------
    # RRF formula: score(i) = Σ_j  w_j / (k + rank_j(i))
    # where rank_j(i) is the position of item i in the ranking produced by
    # metric j.  Smaller ranks (better positions) yield larger contributions.
    # ------------------------------------------------------------------
    if similarity_metric == "hybrid_rrf":
        ...
        if embeddings_cache is None:
            ...
            # Fallback: compute embeddings on the fly when no cache is supplied.
            # This is slower but makes the function self-contained.
            embeddings_cache = [
        ...
        # Build per-metric rankings (best score = smallest rank number).
        ...
        for idx, row in enumerate(training_data):
            # Use the maximum component score for the minimum-similarity gate
            # so that an example qualifies if it is strongly lexical OR strongly
            # semantic, even if not both.
    ...
    # Sort by descending similarity (and ascending index as a tie-breaker).
...
def build_fewshot_examples_for_prompt(
    ...
    Each example is rendered in Spanish with the following structure:
      EJEMPLO 1
      ARTICULO:
      <article text>
      ...
      SALIDA_CORRECTA:
      <space-separated ranking, e.g. t3 t1 t9 ...>
    ...
    for idx, row in enumerate(selected_examples, start=1):
        ...
        # Skip incomplete rows so that the model never sees malformed exemplars.
